		...
		public void keyPressed(KeyEvent e) {
			...
			if (lastKeyCode == KeyEvent.VK_ESCAPE) {
				...
				if (EXIT_CELL_EDITOR_ON_ARROW_KEY) {
					// go down when down arrow pressed during edit
					int row = table.getSelectedRow();
					int col = table.getSelectedColumn();
				...
				// go down and/or stop editing when Enter pressed during edit
				e.consume();
				stopCellEditing();
				int row = table.getSelectedRow();
				int col = table.getSelectedColumn();
				...
				// go to left when left arrow pressed during edit at start of text
				...
				// go to right when right arrow pressed during edit at end of text
				...
				if (EXIT_CELL_EDITOR_ON_ARROW_KEY) {
					// go up when up arrow pressed during edit
					int row = table.getSelectedRow();
					int col = table.getSelectedColumn();
					if (inBounds(row - 1, col)) {
						e.consume();
