...
public class KarelWorld extends JComponent {
	...
	public static final Color BODY_COLOR = new Color(250, 233, 148);
	public static final Color TOOLS_COLOR = new Color(34, 248, 250);
	public static final Color EYES_COLOR = new Color(250, 161, 70);
	public static final Color BEEPER_COLOR = new Color(164, 212, 250);//Color.LIGHT_GRAY;
	public static final Color WALL_COLOR = new Color(0, 0, 0); //new Color(165,42,42);
...
//	public static final Color BEEPER_COLOR = Color.LIGHT_GRAY;
	...
	public void drawCorner(Graphics g, int x, int y, Point pt) {
		...
		if (color != null) {
			...
			// Make colors more light, except grayscale
			if (hsb[1] != 0)
				color = Color.getHSBColor(hsb[0], 0.35f, 0.95f);
				// Make gray colors more lighter
			else if (hsb[2] != 0 && hsb[2] != 1)
				color = Color.getHSBColor(hsb[0], hsb[1], hsb[2] + 0.1f);
		}
	...
	public static void drawFancyKarel(Graphics g, int x, int y, int dir,
		...
		int BODY_SIZE = (int) Math.round(size * 0.5);
		int BODY_OFFSET_X = (int) (-0.1 * size);

		int halfBodySize = (BODY_SIZE + 1) / 2;
		// draw body
		KarelRegion body = new KarelRegion();
		body.setOrigin(x, y, -halfBodySize + BODY_OFFSET_X, -halfBodySize, dir);
		body.addVector(BODY_SIZE, 0, dir);
		body.addVector(0, BODY_SIZE, dir);
		body.addVector(-BODY_SIZE, 0, dir);
		body.addVector(0, -BODY_SIZE, dir);
		g.setColor(BODY_COLOR);
		g.fillPolygon(body.getPolygon());
		g.setColor(Color.BLACK);
		g.drawPolygon(body.getPolygon());
		// draw head
		int HEAD_SIZE = (int) Math.round(BODY_SIZE * 0.5);
		int halfHeadSize = (HEAD_SIZE + 1) / 2;
		KarelRegion head = new KarelRegion();
		head.setOrigin(x, y, -halfHeadSize + BODY_OFFSET_X, -halfHeadSize, dir);
		head.addVector(HEAD_SIZE, 0, dir);
		head.addVector(0, HEAD_SIZE, dir);
		head.addVector(-HEAD_SIZE, 0, dir);
		head.addVector(0, -HEAD_SIZE, dir);
		g.setColor(BODY_COLOR);
		g.fillPolygon(head.getPolygon());
		g.setColor(Color.BLACK);
		g.drawPolygon(head.getPolygon());

		//draw tracks
		int TRACK_SIZE_X = (int) Math.round(BODY_SIZE * 0.8);
		int TRACK_SIZE_Y = (int) Math.round(BODY_SIZE * 0.2);
		int TRACK_LINE1 = (int) Math.round(TRACK_SIZE_X * 0.12);
		int TRACK_LINE2 = (int) Math.round(TRACK_SIZE_X * 0.38);

		KarelRegion leftTrack = new KarelRegion();
		leftTrack.setOrigin(x, y, -TRACK_SIZE_X / 2 + BODY_OFFSET_X, halfBodySize, dir);
		leftTrack.addVector(0, TRACK_SIZE_Y, dir);
		leftTrack.addVector(TRACK_SIZE_X, 0, dir);
		leftTrack.addVector(0, -TRACK_SIZE_Y, dir);
		leftTrack.addVector(-TRACK_SIZE_X, 0, dir);
		g.setColor(TOOLS_COLOR);
		g.fillPolygon(leftTrack.getPolygon());
		g.setColor(Color.BLACK);
		g.drawPolygon(leftTrack.getPolygon());
		drawTrackLine(g, x, y, dir, BODY_OFFSET_X - TRACK_LINE1, halfBodySize, TRACK_SIZE_Y);
		drawTrackLine(g, x, y, dir, BODY_OFFSET_X - TRACK_LINE2, halfBodySize, TRACK_SIZE_Y);
		drawTrackLine(g, x, y, dir, BODY_OFFSET_X + TRACK_LINE1, halfBodySize, TRACK_SIZE_Y);
		drawTrackLine(g, x, y, dir, BODY_OFFSET_X + TRACK_LINE2, halfBodySize, TRACK_SIZE_Y);

		KarelRegion rightTrack = new KarelRegion();
		rightTrack.setOrigin(x, y, -TRACK_SIZE_X / 2 + BODY_OFFSET_X, -halfBodySize - TRACK_SIZE_Y, dir);
		rightTrack.addVector(0, TRACK_SIZE_Y, dir);
		rightTrack.addVector(TRACK_SIZE_X, 0, dir);
		rightTrack.addVector(0, -TRACK_SIZE_Y, dir);
		rightTrack.addVector(-TRACK_SIZE_X, 0, dir);
		g.setColor(TOOLS_COLOR);
		g.fillPolygon(rightTrack.getPolygon());
		g.setColor(Color.BLACK);
		g.drawPolygon(rightTrack.getPolygon());
		drawTrackLine(g, x, y, dir, BODY_OFFSET_X - TRACK_LINE1, -halfBodySize, -TRACK_SIZE_Y);
		drawTrackLine(g, x, y, dir, BODY_OFFSET_X - TRACK_LINE2, -halfBodySize, -TRACK_SIZE_Y);
		drawTrackLine(g, x, y, dir, BODY_OFFSET_X + TRACK_LINE1, -halfBodySize, -TRACK_SIZE_Y);
		drawTrackLine(g, x, y, dir, BODY_OFFSET_X + TRACK_LINE2, -halfBodySize, -TRACK_SIZE_Y);
		// draw eyes
		int EYE_SIZE_X = (int) Math.round(BODY_SIZE * 0.08);
		int EYE_SIZE_Y = (int) Math.round(BODY_SIZE * 0.14);
		int EYE_DIST = (int) Math.round(BODY_SIZE * 0.08);
		KarelRegion leftEye = new KarelRegion();
		leftEye.setOrigin(x, y, halfHeadSize + BODY_OFFSET_X - 1, EYE_DIST / 2, dir);
		leftEye.addVector(EYE_SIZE_X, 0, dir);
		leftEye.addVector(0, EYE_SIZE_Y, dir);
		leftEye.addVector(-EYE_SIZE_X, 0, dir);
		leftEye.addVector(0, -EYE_SIZE_Y, dir);
		g.setColor(EYES_COLOR);
		g.fillPolygon(leftEye.getPolygon());
		g.setColor(Color.BLACK);
		g.drawPolygon(leftEye.getPolygon());
		KarelRegion rightEye = new KarelRegion();
		rightEye.setOrigin(x, y, halfHeadSize + BODY_OFFSET_X - 1, -EYE_DIST / 2 - EYE_SIZE_Y, dir);
		rightEye.addVector(EYE_SIZE_X, 0, dir);
		rightEye.addVector(0, EYE_SIZE_Y, dir);
		rightEye.addVector(-EYE_SIZE_X, 0, dir);
		rightEye.addVector(0, -EYE_SIZE_Y, dir);
		g.setColor(EYES_COLOR);
		g.fillPolygon(rightEye.getPolygon());
		g.setColor(Color.BLACK);
		g.drawPolygon(rightEye.getPolygon());
		// draw arms
		int ARM_SIZE_X = (int) Math.round(HEAD_SIZE * 0.45);
		int ARM_SIZE_Y = (int) Math.round(BODY_SIZE * 0.08);
		KarelRegion leftArm = new KarelRegion();
		leftArm.setOrigin(x, y, BODY_OFFSET_X, halfHeadSize, dir);
		leftArm.addVector(0, ARM_SIZE_Y, dir);
		leftArm.addVector(ARM_SIZE_X, 0, dir);
		leftArm.addVector(ARM_SIZE_X, ARM_SIZE_Y, dir);
		leftArm.addVector(ARM_SIZE_X, 0, dir);
		leftArm.addVector(0, -ARM_SIZE_Y, dir);
		leftArm.addVector(-ARM_SIZE_X, 0, dir);
		leftArm.addVector(-ARM_SIZE_X, -ARM_SIZE_Y, dir);
		g.setColor(BODY_COLOR);
		g.fillPolygon(leftArm.getPolygon());
		g.setColor(Color.BLACK);
		g.drawPolygon(leftArm.getPolygon());

		KarelRegion rightArm = new KarelRegion();
		rightArm.setOrigin(x, y, BODY_OFFSET_X, -halfHeadSize, dir);
		rightArm.addVector(0, -ARM_SIZE_Y, dir);
		rightArm.addVector(ARM_SIZE_X, 0, dir);
		rightArm.addVector(ARM_SIZE_X, -ARM_SIZE_Y, dir);
		rightArm.addVector(ARM_SIZE_X, 0, dir);
		rightArm.addVector(0, ARM_SIZE_Y, dir);
		rightArm.addVector(-ARM_SIZE_X, 0, dir);
		rightArm.addVector(-ARM_SIZE_X, ARM_SIZE_Y, dir);
		g.setColor(BODY_COLOR);
		g.fillPolygon(rightArm.getPolygon());
		g.setColor(Color.BLACK);
		g.drawPolygon(rightArm.getPolygon());
		// draw hands
		int HAND_SIZE_X = (int) Math.round(ARM_SIZE_X * 1.2);
		KarelRegion leftHand = new KarelRegion();
		leftHand.setOrigin(x, y, BODY_OFFSET_X + 3 * ARM_SIZE_X, halfHeadSize - 0.5 + 3 * ARM_SIZE_Y, dir);
		leftHand.addVector(HAND_SIZE_X, 0, dir);
		leftHand.addVector(0, -ARM_SIZE_Y, dir);
		leftHand.addVector(-HAND_SIZE_X + ARM_SIZE_Y, 0, dir);
		leftHand.addVector(0, -ARM_SIZE_Y, dir);
		leftHand.addVector(HAND_SIZE_X - ARM_SIZE_Y, 0, dir);
		leftHand.addVector(0, -ARM_SIZE_Y, dir);
		leftHand.addVector(-HAND_SIZE_X, 0, dir);
		g.setColor(TOOLS_COLOR);
		g.fillPolygon(leftHand.getPolygon());
		g.setColor(Color.BLACK);
		g.drawPolygon(leftHand.getPolygon());
		KarelRegion rightHand = new KarelRegion();
		rightHand.setOrigin(x, y, BODY_OFFSET_X + 3 * ARM_SIZE_X, -halfHeadSize + 0.5 - 3 * ARM_SIZE_Y, dir);
		rightHand.addVector(HAND_SIZE_X, 0, dir);
		rightHand.addVector(0, ARM_SIZE_Y, dir);
		rightHand.addVector(-HAND_SIZE_X + ARM_SIZE_Y, 0, dir);
		rightHand.addVector(0, ARM_SIZE_Y, dir);
		rightHand.addVector(HAND_SIZE_X - ARM_SIZE_Y, 0, dir);
		rightHand.addVector(0, ARM_SIZE_Y, dir);
		rightHand.addVector(-HAND_SIZE_X, 0, dir);
		g.setColor(TOOLS_COLOR);
		g.fillPolygon(rightHand.getPolygon());
		g.setColor(Color.BLACK);
		g.drawPolygon(rightHand.getPolygon());
	}
	private static void drawTrackLine(Graphics g, int x, int y, int dir, int dx, int halfBodySize, int TRACK_SIZE_Y) {
		KarelRegion trackLines = new KarelRegion();
		trackLines.setOrigin(x, y, dx, halfBodySize, dir);
		trackLines.addVector(0, TRACK_SIZE_Y, dir);
		g.drawPolygon(trackLines.getPolygon());
	}
	...
	public void save() {
		...
		try {
			...
			while (iterator.hasNext()) {
				...
				int nBeepers = karel.getCrystalsInBag();
				String str = (nBeepers == INFINITE) ? "INFINITE" : ""
						+ nBeepers;
				if (getKarelCount() == 1) {
					wr.println();
	...
	public void setInteractiveMode(boolean value) {
		...
		if (value) {
			...
			message += "----------------------------\n";
			message += "Up Arrow = go\n";
			message += "Left Arrow = left\n";
			message += "Right Arrow = right (SuperKarel only)\n";
	...
	private void karelCommand() {
		...
		if (lastKarel != null) {
			...
			lastKarel.setCrystalsInBag(nBeepers);
		}
	}
