 ...
 * individual robot that can go inside a world laid out as a grid of streets
 ...
 * 
 * <dl>
 * <dt><code><a href="#go()">go</a>()</code></dt>
 * <dd>Moves this Karel forward one block. Karel cannot go forward if there is
 * a wall blocking the way.</dd>
 * <p>
 * <dt><code><a href="#left()">left</a>()</code></dt>
 * <dd>Rotates this Karel 90 degrees to the left (counterclockwise).</dd>
 * <p>
 * <dt><code><a href="#get()">get</a>()</code></dt>
 ...
 * execute a <code>get</code> instruction only if there is a beeper on
 * the current corner.</dd>
 * <p>
 * <dt><code><a href="#put()">put</a>()</code></dt>
 * <dd>Takes one beeper from this Karel's beeper bag and deposits it on the
 * current corner. Karel can execute a <code>get</code> instruction only
...
public class Karel extends Program implements KarelInterface, Runnable {
	public static final boolean IS_RUS = true;
	/* Private constants */
	...
	static boolean isSuperKarel = false;
	public static boolean isSuperKarel() {
		return isSuperKarel;
	}
	/* Constructor */
	public Karel() {
	...
	public boolean crystalsInBag() {
		checkWorld("crystalsInBag");
		return getCrystalsInBag() > 0;
	}

	public boolean crystalsOnCell() {
		checkWorld("crystalsOnCell");
	...
	public void go() {
		...
		if (world.checkWall(x, y, dir)) {
			throwErrorException(IS_RUS ? "Ошибка. Робот врезался в стену" : "Karel is blocked");
		}
	...
	public boolean noCrystalsOnCell() {
		checkWorld("noCrystalsOnCell");
		return world.getBeepersOnCorner(x, y) == 0;
	}

	public boolean noCrystalsInBag() {
		checkWorld("noCrystalsInBag");
		return getCrystalsInBag() == 0;
	}
	...
	public void get() {
		...
		if (nCorner < 1) {
			throwErrorException(IS_RUS ? "Ошибка. get: В этой точке нет кристаллов" : "get: No beepers on this corner");
		}
	...
	public void put() {
		...
		int nBag = getCrystalsInBag();
		if (nBag < 1) {
			throwErrorException(IS_RUS ? "Ошибка. put: У робота в сумке нет кристаллов" : "put: No beepers in bag");
		}
		nBag = KarelWorld.adjustBeepers(nBag, -1);
		setCrystalsInBag(nBag);
	...
	public void setCrystalsInBag(int nBeepers) {
		beepers = nBeepers;
	}
