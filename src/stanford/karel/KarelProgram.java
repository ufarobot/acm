...
public class KarelProgram extends Program {
	...
	public KarelProgram() {
		...
		// set up status label
		statusLabel = new JLabel(IS_RUS?"Для выполнения программы нажмите \"Запустить Программу\"":"Welcome to Karel!");
	...
	public void setStatus(String status) {
		if (status.contains("Running")||status.contains("Выполняется")) {
			statusLabel.setForeground(DARK_YELLOW);
		} else if (status.contains("due to an error")||status.contains("Работа программы прервана из-за ошибки")) {
			statusLabel.setForeground(DARK_RED);
		} else if (status.contains("Finished")||status.contains("Выполнение закончено")) {
		...
		while (true) {
			...
			try {
				if (karel == null) {
					...
					setStatus(IS_RUS?"Выполнение закончено":"Finished running.");
				}
				world.setRepaintFlag(true);
				world.repaint();
			} catch (Exception ex) {
			...
			if (throwable == null) {
				...
				setStatus(IS_RUS?"Работа программы прервана из-за ошибки.":"Halted due to an error.");
