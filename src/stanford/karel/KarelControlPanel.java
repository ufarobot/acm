...
import static stanford.karel.Karel.IS_RUS;
...
public class KarelControlPanel extends CardPanel implements KarelWorldMonitor,
	...
	private JButton saveAsWorldButton;
	...
	protected JComponent createEditorPanel() {
		...
		vbox.add("", editor);
		saveWorldButton = new JButton(IS_RUS?"Сохранить Мир":"Save World");
		...
		saveAsWorldButton = new JButton(IS_RUS?"Сохранить Мир":"Save World");
		saveAsWorldButton.setMnemonic('S');
		saveAsWorldButton.addActionListener(this);
		vbox.add("/width:" + BUTTON_WIDTH + "/height:" + BUTTON_HEIGHT
				+ "/top:" + BUTTON_SEP, saveWorldButton);
		dontSaveButton = new JButton(IS_RUS?"Не сохранять":"Don't Save");
	...
	protected JComponent createButtonPanel() {
		...
		//by Airat
		startButton = new JButton(IS_RUS?"Запустить Программу":"Start Program");
		startButton.setMnemonic('S');
		startButton.addActionListener(this);
		resetButton = new JButton(IS_RUS?"Сбросить Программу":"Reset Program");
		resetButton.setMnemonic('R');
		resetButton.addActionListener(this);
		loadWorldButton = new JButton(IS_RUS?"Загрузить Мир":"Load World");
		loadWorldButton.setMnemonic('L');
		loadWorldButton.addActionListener(this);
		newWorldButton = new JButton(IS_RUS?"Создать Мир":"New World");
		newWorldButton.setMnemonic('N');
		newWorldButton.addActionListener(this);
		editWorldButton = new JButton(IS_RUS?"Изменить Мир":"Edit World");
		...
		// by Airat
		hPanel.add("/center", new Label(IS_RUS?"Медленно":"Slow "));
		hPanel.add("/center/width:100", speedBar);
		hPanel.add("/center", new Label(IS_RUS?"Быстро":" Fast"));
	...
	protected JComponent createResizePanel() {
		VPanel vbox = new VPanel();
		cancelButton = new JButton(IS_RUS?"Отменить":"Cancel");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (source == startButton) {
			program.setStatus(IS_RUS?"Выполняется ...":"Running ...");
			...
			program.setStatus(IS_RUS?"Загрузка начального состояния":"Reset to initial state.");
			...
			if (fileName != null) {
				...
				program.setStatus((IS_RUS?"Мир загружен из ":"Loaded world from ") + fileName + ".");
			}
			...
			if (world.getPathname() == null) {
				...
				if (fileName != null) {
					...
					program.setStatus((IS_RUS?"Мир сохранен в ":"Saved world to ") + fileName + ".");
				}
			}
			world.save();
			...
			if (fileName == null) {
				...
				if (world.getKarelCount() == 1) {
					...
					karel.setCrystalsInBag(KarelWorld.INFINITE);
					world.repaint();
				}
				editor.initEditorCanvas();
				setView("editor");
